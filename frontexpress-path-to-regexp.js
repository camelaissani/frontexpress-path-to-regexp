...
var index = {
    ...
    plugin: function plugin(application) {
        var decodeParmeterValue = function decodeParmeterValue(encodedValue) {
            var v = decodeURI(encodedValue);
        ...
        application.set('route matcher', function (request, route) {
            ...
            var keys = [];
            var re = index$1(route.uri, keys);

            // check request path against route regexp
            var routeMatch = baseUriToCheck.match(re);
            if (!routeMatch) {
                return false;
            }
